...
class Patcher(object):
    ...
    def ffrange(self, start, end):
        ...
        ll = end - start
        ss = start - self.offset

        if ss > self.length:
            toadd = (ss-self.length) + ll
            print("ss > self.length, adding  %08x" % (toadd))
            self.bytes += '\xFF'*(toadd) 
            self.length = len(self.bytes)

        elif (end - self.offset) > self.length:
            toadd = (end - self.offset) -self.length 
            print("end > len, adding  %08x" % (toadd))
            self.bytes += '\xFF'*(toadd) 
            self.length = len(self.bytes)
